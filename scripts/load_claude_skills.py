...
def register_skill_to_agentscope(skill_metadata: Dict) -> bool:
    ...
    try:
        ...
        toolkit = Toolkit()
        toolkit.register_agent_skill(skill_dir=str(skill_path))
...
示例:
  ...
  python scripts/load_claude_skills.py --generate-only
    ...
    parser.add_argument(
        "--generate-only", action="store_true", help="仅生成注册代码，不实际注册到 AgentScope"
    )
    ...
    print()
    print(
        f"📊 统计: 发现 {len(external_skills)} 个 Claude Skills, {len(internal_skills)} 个内部 Skills"
    )
